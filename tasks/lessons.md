...
- **2026-08-20 — "merged" in chat is not merged on GitHub; and deleting a head branch closes its open PR.** Ron said PR #103 was merged; the session deleted the remote feature branch per rule 5 — but the merge had never landed (`state: CLOSED`, `mergedAt` empty, `main` unmoved), and GitHub responded to the branch deletion by **auto-closing the open PR**. Recovery was clean only because the commit survived in the session's worktree: re-push the same branch, `gh pr reopen`, re-verify checks. The rule that would have prevented it already exists ("verify state right before touching git") — the miss was scoping it to *local* state. **Pattern:** before `git push origin --delete <branch>`, require proof of merge from the remote itself — `gh pr view N -q .mergedAt` non-empty, or the merge commit visible in `git log origin/main` — in the SAME command as the deletion. A user saying "merged" (or a UI that looked like it) is a claim about the remote, not evidence of it.
- **2026-08-21 — Max's in-thread "On it — I'll filter that out going forward" was a hallucinated commitment; only a code PR stops deterministic cron output.** Ron asked Max to suppress QA test leads from the unclaimed-lead nag; Max promised a filter and the nag kept firing (11:30, 12:30, 14:00) because the nag is plain cron code Max cannot modify — his promise changed nothing and read as a lie to the team. **Pattern:** when Max conversationally promises to change how a *scheduled/deterministic* output behaves, treat that as an open bug report, not a resolution — the fix must land in index.js via PR. Worth a future guardrail: teach Max's system prompt to say "I'll flag this for Ron/engineering" instead of promising self-modification he doesn't have. Related: name-based suppression filters that HIDE data must match whole words, not substrings, and must log every suppression (same silent-blind-spot reasoning as the Adrian RM 11-day miss).
- **2026-08-21 — The "CI names its test files explicitly" lesson is dead; ci.yml globs `test/*.test.js` now.** The recipe-standard skill and the unattended registry both still warned that a new test file must be added to ci.yml by name or it never runs. Verified while shipping PR #109: the workflow deliberately switched to a glob (its own comment says the explicit list kept forgetting files), so `test/open-deal-followup.test.js` ran in CI with zero workflow edits. Registry corrected. Pattern: a lesson about a specific file's contents carries an implicit as-of date — before acting on one, read the file it describes; the codebase fixing the root cause is what lessons cannot see on their own.
- **2026-08-22 — "Tomorrow" is not a cron field: compute the weekday before promising a firing time.** A session shipped a weekdays-only cron on a Thursday evening, told Ron the first dry run was "tomorrow 10 AM", and scheduled its own verification for Friday—except the merge landed late Thursday CR and "tomorrow" was actually Saturday by the session clock's UTC date, so the verifier woke up on a weekend hunting a `1-5` cron that correctly hadn't run. Ten minutes of false-alarm diagnosis (empty logs, empty activity rows, wrong-Supabase detour) for a date-math slip. Pattern: whenever stating when a schedule fires next, derive the day-of-week from the cron expression + timezone explicitly (a `date -d` one-liner or the DB's `to_char(now() AT TIME ZONE 'America/Costa_Rica', 'Dy')`), and cross-check any "tomorrow/today" wording against it. Corollary confirmed the same morning: absence of a `1-5` cron's run on a weekend is health, not failure — check the day before diagnosing.
- **2026-08-24 — Before mass-mutating data, measure what it IS. The "299-deal backlog" was 263 rows of frozen import history.** Ron approved a bulk-triage of the open-deal book and the instinct was to start writing `lost` to hundreds of rows. Grouping by `source` first took ten minutes and dissolved the whole task: 263 of 299 were pre-cutover iClosed rows with no GHL card behind them (107 from one import on 2026-05-19; one prospect literally named "Prueba"). The fix was a `WHERE` clause, not a migration — and the irreversible write that was nearly made would have polluted lost-reason analytics with test records. **Pattern:** when a instruction implies a large or irreversible write, the first action is a characterising query (`GROUP BY source/date/linkage`), not the write. If the data has a shape nobody predicted, the task usually changes. Corollary: a number that surprises you (299 open deals for 3 closers) is a hypothesis about the data, not a fact about the business.
- **2026-08-24 — A new function that reads a shared table must inherit the filters its siblings already apply.** `fetchOpenDeals` queried `revops_sales_outcomes` without the `GHL_CUTOVER_ISO` floor that `fetchDueSalesCalls` had carried since the cutover, so the sweep treated pre-migration history as a live book and would have carded six un-actionable ghosts on its first live day. The gate existed, was documented, and had a comment explaining exactly this failure mode — it just wasn't copied. **Pattern:** before shipping a query against a domain table, grep for every other function reading that table and diff their `WHERE` clauses against yours. Each divergence is either a deliberate decision worth a comment, or a bug. Same class as the `merge=union` and `TASK_HEADERS` lessons: the guardrail usually already exists somewhere in the file.
- **2026-08-24 — Two silent-failure gotchas, both found in production, both invisible to `node --check` and to green tests.** (1) **Node's `fetch` has no default timeout.** A GHL request that never answered blocked its caller for the entire 600s cron budget, produced zero log lines, and was abandoned — indistinguishable from a hang anywhere else in the job. Any outbound HTTP in an unattended job needs an explicit `AbortSignal.timeout`, and the timeout should be retryable like a 5xx. (2) **`node-postgres` returns `timestamptz` as a Date OBJECT, so `String(date)` is `"Mon Jun 29 2026 …"`** — sorting those with `localeCompare` orders by weekday name, then MONTH NAME, putting `"Jul"` before `"Jun"`. Ages computed with `Date.parse` were correct, so nothing looked broken; only the ORDER was wrong, and the order was the product. Compare timestamps as instants, and write the fixture with real Date objects — ISO strings sort correctly and hide the bug.
- **2026-08-25 — A PR left open while main moves will not merge: `BEHIND` is a ruleset block, not a conflict.** PR #114 was green, MERGEABLE and two days old; `gh pr merge` refused it with "requirements have not been met" and no obvious reason. `gh pr view --json mergeStateStatus` returned `BEHIND` — the ruleset requires branches be up to date with `main`, and seven PRs had landed since. The fix is a rebase, not `--admin`. **Pattern:** when a merge is refused but `mergeable` is `MERGEABLE`, read `mergeStateStatus` before reaching for a force flag — `BEHIND` means rebase, `BLOCKED` means a required check or review, `DIRTY` means a real conflict. And rebasing a stale branch is never just a fast-forward: re-run the full suite afterwards, because the branch was tested against a `main` that no longer exists (here index.js had changed under it in four places).
- **2026-08-25 — A behavioural constant enforced at more than one call site needs a test pinning BOTH the value and the indirection.** `OPEN_DEAL_CARDS_PER_CLOSER_PER_RUN` gated two branches of the sweep and had no assertion at all, so a future edit could hardcode a literal at one site and leave the constant lying about what the system does — the divergence would be invisible to `node --check`, to every existing test, and to review. The added check asserts the value, a sane range (it is a burst guard, so 1–10, not an arbitrary quota), and that every enforcement site reads the constant rather than a number. **Pattern:** grep the constant's own name before shipping a change to it; if it appears at N call sites, the test must prove all N still reference it.

- **2026-08-26 — `bot_id` means "a bot posted this", never "*we* posted this". Identity checks must compare a specific id.** `isMaxMessage()` treated any `m.bot_id` as Max's own post, so every other app in the workspace — Make, above all — was indistinguishable from him. The blast radius was nothing like the size of the line: Max claimed ownership of every Make booking card thread and answered every human reply in it, the learning loop extracted "report lessons" from Make's text, and thread transcripts rendered app posts as `Team Member`. It read as a *behaviour* problem ("Max is too chatty") for as long as nobody checked the predicate. **Pattern:** any "is this mine?" test against a third-party payload must compare against a concrete identity resolved from the platform (`auth.test` → `bot_id`), not against the mere presence of a type marker. And resolve that identity **before** opening the event socket — resolving it after `start()` leaves a window where events are handled with the bot's own identity unknown. When it cannot be resolved, prefer the answer that makes the agent do *less*.
- **2026-08-26 — When an agent is "too chatty", check what it thinks it is replying to before touching the prompt.** The instinct on "Max responds to every thread with no value added" is to write a system-prompt rule telling him to be more selective. That would have been a slow, unverifiable fix layered on top of a live bug, and it would have failed: he was not choosing to speak, he was being *told* every Make thread was his own conversation. **Pattern:** for any "the agent behaves badly" report, first read the gate that decides whether it speaks at all and confirm the inputs to that gate are true. Prompt tuning is the last resort, not the first — and when a deterministic gate and a model judgement are both warranted, let the gate narrow the candidates and the model decide only within them (here: gate → "could plausibly be for Max", model → `NO_REPLY` for the rest). Log the model's silences; a suppression nobody can count is indistinguishable from a handler that crashed.
- **2026-08-25 — A source-level test that slices the wrong range passes forever and guards nothing.** The check "the digest makes no GHL calls" was written as `SRC.slice(fnStart, SRC.indexOf('cron.schedule', fnStart))` — but the cron registrations sit hundreds of lines later, so the slice swallowed several unrelated functions that legitimately call `ghlFetch`, and the assertion failed for a reason that had nothing to do with the digest. Had the polarity been reversed (asserting presence rather than absence) it would have passed forever while checking nothing. **Pattern:** any test that slices source by markers must assert the slice itself — a length bound, or that the boundary marker was actually found — and must be made to fail on purpose before it is trusted. The extraction tests in this repo all read to a `// ─── end …` marker for exactly this reason; ad-hoc boundaries need the same discipline.
- **2026-08-25 — When a filter changes what a table means, every consumer that compares against an external system needs re-deriving, not just re-reading.** Flooring `fetchOpenDeals` on `GHL_CUTOVER_ISO` (#120) silently invalidated the digest's GHL cross-check: it compared GHL's Open-Deal stage, which still contains every pre-cutover card, against a portal book that now excludes them, so it would have reported ~28 false "logged outside Max" every week. Nothing errored; the number was just wrong. **Pattern:** after narrowing a query, list every other computation that joins that result against a system with different scope, and re-derive each — a cross-system check is only meaningful when both sides describe the same population. A comparison that survives a scope change without being revisited is a false alarm waiting for a schedule.

- **2026-08-26 — `say` is a surface, not a person. A handler shared between DMs and channels must be told which one it is.** `handleDraftReply` was written when drafts only happened in DMs, where `say` *is* the private channel to the requester. Once the same function ran from the channel and `app_mention` handlers, the identical line became a broadcast — and what it broadcast was specifically the content that had been routed for approval *because* it was not safe to publish. Nothing in the type of `say` changes between the two; only the caller knows. **Pattern:** any shared helper that emits sensitive content must take an explicit surface flag rather than infer one, and that flag defaults to the **private** value so a call site added later fails closed. Grep for the sibling paths before shipping the fix — `handleSetterReview` had the same bug and had simply never been triggered from a channel yet. The correct implementation was already in the repo (`promoteDraftToRon` DMs the body and acks without it); the fix was making two paths match a third, not inventing anything.
- **2026-08-26 — A post-mortem written from the symptom will name the wrong cause; re-derive it from code before acting on it.** The PR #127 note recorded this leak as Max "narrating instead of emitting the sentinel" — a plausible read of the Slack transcript, and wrong. Acting on it would have meant tightening prompt instructions against a model that was already behaving correctly, leaving the actual defect shipped and the "fix" unfalsifiable. Reading `handleDraftReply` took two minutes and the leaked string was sitting in it as a template literal. **Pattern:** when a hand-off note attributes a failure to model behaviour, treat that as a hypothesis and find the code path that produced the exact observed output before changing anything — an output you can find verbatim in a template literal was never a model decision. This applies to notes written by an earlier session of your own.

- **2026-08-26 — "Zero rows" is a finding, not a clean bill of health. Ask why the table is empty before concluding nothing is wrong.** The audit was looking for *poisoned* report-lessons and found none — which reads like good news until you notice the table has **no rows at all, in either direction, since the feature shipped in April**. A learning loop that has never learned anything is a worse outcome than one learning the wrong things, and it is far quieter: nothing errors, no alert fires, the reports just quietly carry no lessons and look exactly like reports that had nothing to learn. **Pattern:** when a write path returns an empty result set, distinguish "the condition was never met" from "the condition can never be met" — reconstruct one real event that *should* have produced a row and prove the row exists. Here the Aug 18 thread was a textbook trigger (Max-rooted report, Ron tagging with a factual correction) and produced a sibling row but not the lesson row, which located the always-false predicate in one step. Features listed as "verified built" deserve this check specifically: nobody re-tests what the docs already call done.
- **2026-08-26 — Prose in a prompt rots silently; a test is the only thing that makes it fail loudly.** Max's system prompt carried an offer catalog killed 13 days earlier, and nothing anywhere could notice: `node --check` passes, every test passed, review reads it as plausible English. It surfaced only because a build brief in a *different repo* happened to list it as an overdue task. The prompt is the highest-leverage text in the repo — Max briefs the team daily and closers borrow his language, so a wrong price there reaches a prospect through a human, which no output filter catches. **Pattern:** any prompt text that encodes facts with an external source of truth (prices, SKUs, team roster, legal terms) needs an assertion tying it to that source, and the assertion must cover **both** directions — the live facts are present *and* the retired ones are absent. Watch the guardrail trap: a prompt legitimately names dead offers so the model can refuse them, so "must not appear" has to be asked of the prompt **minus** the refusal block, or the guard fights the guardrail. Same family as the TASK_HEADERS and PREAMBLE_GUARD lessons: when prose instructions are load-bearing, machine-check them.

- **2026-08-27 — Check repo visibility BEFORE writing any business fact into source. `gh repo view --json visibility` is two seconds.** Closing an overdue north-star item by pasting the locked price catalog into Max's system prompt felt like pure progress: it was the documented required change, sourced from plan-of-record, tested, reviewed and merged. It also published internal deal structure to the open internet, because **ng-agent is public and the repo the catalog came from is private** — a fact recorded in memory and never checked at the moment it mattered. Nothing in the task framing mentioned visibility; the north-star brief says "internal-only by design", which describes the *agent*, not the *repo*. **Pattern:** treat "which repo is this, and who can read it?" as a precondition for committing any price, contract term, client name, credential-adjacent value or strategy detail — the same reflex as checking the branch before committing. And when a request would move confidential data (here: "make the test read the catalog from the strategy repo", which meant vendoring a private catalog into a public repo), the right first move is the visibility check, not the implementation. The remedy is also asymmetric and worth knowing up front: git history is effectively permanent once public, so prevention is the only real control.
- **2026-08-27 — Point the guard at the whole blast radius, not just the file you were thinking about.** The test written to keep prices out of `index.js` was extended to scan `scripts/` too, almost as an afterthought — and immediately failed on the sync script I had just written, which hardcoded the entire catalog and would have defeated the change it existed to implement. It then failed twice more on things no reviewer would flag: **code comments explaining the leak by quoting the leaked numbers**, and a truthiness bug where a whitespace row counted as a valid empty catalog. Three real defects, all mine, all in the change that was supposed to fix the problem. **Pattern:** when the invariant is "X must not appear anywhere", enumerate *everywhere* that ships — every tracked source file, not the one file in focus — and have the failure name the offending file so the fix is one step. Comments and docs are shipped text and count. And write the guard before believing your own implementation: I would have shipped all three defects, in the PR whose entire purpose was preventing exactly this.

- **2026-08-27 — Before recommending an infrastructure setting change, check what that setting is currently load-bearing for.** "Make the repo private" was offered twice as the clean fix for a public-repo data exposure, and it was wrong: on GitHub Free, rulesets exist only on public repos, so the change would have silently removed PR-required, required-status-checks, force-push and deletion protection from a branch that auto-deploys to production. Nothing warns you — the ruleset simply stops applying, and the repo looks fine. The recommendation was made from the security angle alone, without asking what else depended on `public`. **Pattern:** a settings change is a change to every invariant that setting hosts. Before proposing one, enumerate what currently enforces the repo's guarantees (`gh api .../rulesets`, branch protection, required checks, deploy integrations) and verify each survives — and prefer *proving* it on a comparable object over reasoning from documentation: four sibling private repos returning 403 settled the plan question in one command, where "I think Free might not support this" would have stayed a guess. Also worth separating in your own advice: what a remedy prevents going forward versus what it retracts. Here it retracted nothing — the data was already public — which alone should have lowered its priority against the cost.

- **2026-08-30 — Fixing the cause you found is not the same as fixing the feature. Trace the whole chain before declaring a loop alive.** PR #127 corrected the predicate that stopped report-lessons ever being written, and that was reported as reviving the learning loop. It was not: extraction still only ran on `app_mention` (never delivered for DMs), the two reports whose lessons were read most were themselves DMs, and the id written did not match the id read for six of the sales reports. Each defect alone was sufficient to keep the loop dead, and the first one masked the rest — with zero rows, there was nothing downstream to observe. **Pattern:** for a read-write feature, verify the **whole** chain before claiming it works — who writes, with what key, who reads, with what key, and on which surfaces each runs. Ask specifically "can the writer ever fire for the thing the reader reads?", because a read whose writer lives on an unreachable code path fails silently and forever. And when a fix is deployed but the triggering event has not occurred yet, say **"deployed, unproven"** and name the event that will prove it — not "fixed".
- **2026-08-30 — Mutation-test the guard, not just the code; a passing assertion can still be blind.** The new test asserted that every report id READ has a WRITER — the check designed to make the dead-read defect impossible. It passed, and it was insufficient: reverting one of the two dynamic-cron reads to channel scope scored **zero failures**, because the id set still matched via the *other* read. A set comparison cannot see a regression that preserves the set. Only mutating the code and watching the suite stay green exposed it; the fix was to pin the **count** and ban channel variables in the call outright. **Pattern:** after writing a guard, reintroduce each defect it claims to prevent and confirm it goes red. Aggregate assertions (sets, "contains", "at least N") are the ones most likely to pass while blind — prefer exact counts and structural position checks where the invariant is really about *every* site, not *some* site.

- **2026-08-31 — Autonomy is earned by the guardrails being code, not the model being trusted.** "Full autonomy, no dry run" was shippable in a day ONLY because every safety property is a deterministic, testable function: evidence dates are parsed and counted (a model can assert "3 weeks" in one breath — one date claimed three times is one day), the nightly action cap is a hard integer, dedup is a timestamp comparison, and the action vocabulary is a Set. The LLM never posts raw to a team surface: the brief's factual sections are built by code, the interpretive sections are parsed through a line protocol, and generation failure degrades to an honest in-place sentence rather than a silent skip or posted garbage. **Pattern:** when granting an agent autonomy, put every property you would want to assert in an incident review into pure functions FIRST, pin each with a test, and mutation-verify each one actually fires when removed. The prompt states intent; the code enforces it. Corollary: make the autonomy *visible* — every auto-action lands in an audit DM and the weekly brief's ACTIONS ALREADY TAKEN section, because invisible autonomy is indistinguishable from a rogue agent the day something goes wrong.
- **2026-08-31 — A mutation harness can lie in the same direction as the tests it checks: count crashes as red.** Two guardrail-removal mutations reported "0 red" — they had crashed the test file (TypeError on `dropped[0].reason` when the drop never happened) and the counter only greps lines starting with FAIL, so a crash read as a clean pass. The guardrails were fine; the meta-layer was blind, in exactly the way that would have let a future regression through. **Pattern:** a mutation run has three outcomes — red, green, and CRASH — and CRASH must be treated as red (something detected the mutation, just not gracefully). Grep exit codes, not only output lines. And write assertions with optional chaining (`d.dropped[0]?.reason`) so a missing element FAILS the check instead of throwing past the remaining checks — a thrown test stops counting everything after the throw, which under-reports by an unknown amount.

## 2026-08-31 — A duplicated allowlist is a scheduled outage; a content check cannot see what was never ingested

**What happened:** five days of one setter's GHL bookings never reached `revops_appointments` and no check fired. The new ONLY RON calendar was in NONE of the three hardcoded sales-calendar lists (dash webhook allowlist, dash hydration search, ng-agent appt-status sweep) — each a copy of the same fact, each maintained by hand, and the codebase even carried a comment (2026-08-18) listing ONLY RON as a sales calendar while the lists beside it disagreed. The env-gated drift check (`GHL_SALES_CALENDAR_IDS_EXPECTED`) that would have caught it was never armed — a check that exists but is off is indistinguishable from no check.

**Patterns to keep:**
1. **Derive shared facts from their source of truth, don't copy them.** The calendar set now comes live from the GHL calendar group at both dash call sites; the static list survives only as a fail-closed floor. When a fact must exist in two places, one of them should be a machine-checked mirror of the other — and the checker must be on by default, not env-gated off.
2. **Every ingestion pipeline needs one check that starts from the OTHER side.** All our checks walked ingested rows, so never-ingested data was invisible by construction. `runGhlRevopsReconciliation` reads GHL first and asks "where is your row?" — copy this shape for any source→DB pipeline (`diffGhlEventsAgainstRevops` is the reusable pure diff).
3. **"The workflow fired" ≠ "the data arrived usable."** GHL executed the webhook for every missed booking — but mobile-app-created appointments ship contact-only payloads with no appointment object. Verify pipelines against data produced by REAL user paths (mobile manual booking), not only the path your own systems exercise (widget bookings) — same lesson as the #144 `bodyText` HTML bug, one week apart.
4. **A report window must anchor to its cadence, not the calendar day.** Any digest that runs at H o'clock and counts "today" has a dead zone from H to midnight, permanently. Window = last-run → now (overlap is fine, holes are not) — `computeEodBookedWindow` is the reference.

- **2026-08-31 — A report that concatenates accumulated data is a report nobody reads (gap-report grooming, [PR #160](https://github.com/neurogrowth-cr/ng-agent/pull/160)).** The Monday gap report was correct and complete, and Ron's verdict was *"nobody is going to read this... I'd rather not post anything."* Correct-but-unread is net-negative — the channel learns to skip Max's posts. The two anti-patterns to never repeat on any report surface: **(1) never render a raw list when a count tells the story** (20 blocked activity titles → "20 steps blocked, oldest is X"); **(2) never join accumulated knowledge rows into output** — date-suffixed keys like `client:<slug>:<date>` produce near-duplicate restatements *by design*, so joining `getClientContext()` results multiplies the same sentence five ways; take the newest row and clip it. Ron's format bar, now standing policy for every Max surface: flowy sentences under real bold headers with blank lines, no `|`/`·` fragment chains, no per-line emoji tags, and a **real data point** (the week-streak counter turned "Capital Brokers is blocked again" into "12th week on this list" — the difference between noise and an escalation case). Enforce format in code with extract-tests, not in prose guidance — same lesson as the game-plan brief (PR #149), reconfirmed. And when a chronic item repeats N weeks unchanged, the report should stop describing it and start asking for a decision.

- **2026-08-31 — Known state is not news: report transitions, not standing rosters (gap report v2, [PR #162](https://github.com/neurogrowth-cr/ng-agent/pull/162)).** PR #160 groomed the *format* (one line per client, counts over dumps) and Ron still flagged it the same day: re-listing clients everyone knows are customer-blocked is noise even when each line is short — *"team already knows it... remove noise and only focus on signal."* The durable rule for every recurring Max surface: **the unit of report is the delta, not the roster.** Diff this run against a persisted state row; name only what changed (new, crossed a threshold, resolved) plus what is on *our* side to fix (stale assignees, missing blocker notes); collapse unchanged known state to a count with a pointer to the on-demand tool; **post nothing when nothing changed** — silence is a feature that keeps the channel trusting the reports that do arrive. Threshold asks (escalate-or-close at 60 days) fire **once at the crossing**, not weekly — a repeated ask trains people to ignore it. Two implementation disciplines that made this safe: persist state on successful post *or* clean skip but never on a failed post (otherwise a Slack outage silently eats a week's transitions), and suppress transition phrasing on the first run (a fresh baseline is not "8 new clients"). Related: daily EOD already had this idea (`getPortalAlerts` daily mode drops standing blocked) — the weekly report just never inherited it.

- **2026-09-01 — When verifying an audit trail, check the thing the note claims happened, not just that the note itself posted.** The pattern-reflection audit post in #ng-pm-agent said "Created P1 Notion task: X" twice. Trusting that line would have been trusting the LLM's own report of its actions — exactly the failure mode the whole guardrail system exists to prevent elsewhere. Ran `notion-search` for both titles before reporting the run as verified; both existed, in Operations Tracking, with the evidence dates in the note as specified. **Pattern:** an audit log entry is a claim, not a fact, until the claimed side effect is independently confirmed in the system it claims to have touched — especially for a newly-shipped autonomous-action path on its first live run, where "the code posted a message saying it worked" and "it worked" have not yet been shown to be the same thing.

- **2026-09-01 — A monitor's non-findings must never be able to make it speak ([PR #166](https://github.com/neurogrowth-cr/ng-agent/pull/166)).** The cron liveness audit reported three brand-new crons as "never logged" under a ⚠️ header, while its own body correctly explained each was not due yet. The wording was honest and the alarm was still wrong: the two weekly ones would have raised the same ⚠️ every morning for a week with nothing behind it. The file's own comment already knew the stakes — *"an audit that posts every day teaches you to skim it"*, and a muted audit is worse than none because it still looks like coverage. **Pattern:** when a check can produce a "this is fine, here's why" result, that result must be excluded from the count that decides whether to post, not merely worded gently inside the post. Same bar as the report grooming rule: name the delta, not the roster. **Second pattern, the more dangerous half:** when adding a "not a problem yet" exemption, ask what happens if the excusing condition renews forever. Here the exemption was scoped to `process.uptime()`, and a job that has never succeeded is invisible to the staleness check by construction — so on a service that redeploys often, a weekly job that had never once worked would be excused indefinitely. **An exemption needs a clock that a restart cannot reset**; this one now also measures how long the audit itself has observed the job silent, read out of its own activity history. **Third:** that history-derived backstop reads a field the audit writes (`output.silent`). Narrowing that field to just the findings would have looked like a tidy-up and silently disabled the backstop with no test failing — so it is pinned by an explicit source-level test with a comment saying why.

- **2026-09-18: a "done" marker is a claim about the past, not a guarantee about the present ([PR #206](https://github.com/neurogrowth-cr/ng-agent/pull/206)).** Max stamped Danny Gómez's outcome card `confirmed` after a successful insert, then treated that state as permanent. A day later dash legitimately deleted the row (stale no_show after the appointment moved), and Max's own marker made the call unanswerable: the cron skipped it, the card ignored taps, and only Ron's escalation kept counting, 14 times. **Pattern:** when Max records that it wrote something to a system it does not own, the marker must be re-validated against that system before it is used to suppress work. Another writer can undo the row at any time. **Second pattern:** an escalation that keeps firing while the person being escalated has no way to act is a bug in the loop, not a discipline problem. Before blaming the closer, check that the action surface still works. **Third pattern:** the first theory (dash silently deletes outcomes) was wrong, and one query against `ghl_webhook_deliveries` showed it. Read the audit trail before designing a fix for a suspected cause.

- **2026-09-20: a write path with no test that reaches the database is untested, however many tests surround it ([PR #208](https://github.com/neurogrowth-cr/ng-agent/pull/208)).** `promoteOpenDealOutcome` shipped with five SQL placeholders and four values, and failed on its first real call and every call after for three weeks. The feature had a full test file, but it covered the pure decision functions around the query and never the query. **Pattern:** when a test cannot run the SQL, check it statically. Counting placeholders against bound values is cheap, needs no database, and would have failed this PR on day one (`test/sql-placeholders.test.js` now does it for every inline query). **Second pattern:** a failure marker must never double as a completion marker. The warning stamp meant "the write failed", and the handler read it as "this card is handled", so one bug became a permanently dead card. A retryable failure and a finished action need different signals. **Third pattern:** an error message written for one cause misleads when it fires for another. "Tell Ron the outcome write path is down" sent a code bug looking like an outage. When a catch-all reply names a specific cause, it is wrong for every other cause.

- **2026-09-20: "verified" has to mean verified against the thing that can refuse you ([PR #211](https://github.com/neurogrowth-cr/ng-agent/pull/211)).** #208 fixed a real bug, passed 46 test files, and was reported to Ron as working. His next tap failed, because the database role behind the write had never been allowed to run it. Every check had gone through a fake pg client, which says yes to everything. **Pattern:** for a write path, the systems that can refuse are the database (grants, constraints, RLS) and the API on the other side. A test that replaces them proves the code's shape, never that the write is permitted. Say "unit-tested" when that is what it is, and keep "verified" for a run against the real refuser. **Second pattern: stacked failures hide each other.** The parameter bug failed before the query left the process, so the permission error behind it was invisible for three weeks. After fixing the first failure on a path that has never once succeeded, assume there is another one and go look before announcing. **Third pattern:** production can be probed safely. A single transaction that does the work and ends in a deliberate exception rolls back everything, DDL included, and the exception message carries the results out. That is a real test against real grants and real rows with nothing left behind.
- **2026-09-20: when a report looks wrong, audit the block it was handed before the prompt, and treat every unpaged read of a growing table as a bug with a date on it ([PR #210](https://github.com/neurogrowth-cr/ng-agent/pull/210)).** The Setter Leaderboard said 9 calls were waiting on closers. The model had copied its data block faithfully; the block was wrong in three independent ways. (1) A state with no bucket lands in the default bucket: cancelled calls had nowhere to go, so they became "pending", and "pending" was then narrated as a closer failure. Any tally whose buckets do not add up to the total by construction will misfile whatever it was not told about, so the test now asserts the sum. (2) `setter_claims` crossed PostgREST's 1000-row cap sometime in September. The read was oldest-first, so the rows that disappeared were the newest and most relevant, and the fallback (GHL booker) produced a plausible answer instead of an error. Nothing failed; two calls just changed owner. A `.select()` with no `.range()` on a table that grows every day is correct only until it is not, and it will not say when. (3) The prompt explained a number the code did not mean: "(from N distinct leads) means a prospect rebooked" was true of the intent and false of the implementation, which dropped email-less prospects. When a prompt gives the model a reason for a number, that reason becomes a claim in a public post, so it has to be true of the code, not of the design. Working order that found all three fastest: tally the source rows by hand, compare to the block, then ask the upstream system (GHL status, `ghl_webhook_deliveries`) about each row that looks undecided.

- **2026-09-21: a bucket is a policy decision, so ask what the business calls the thing before inventing a neutral name for it ([PR #214](https://github.com/neurogrowth-cr/ng-agent/pull/214)).** In #210 cancelled calls were pulled out of "pending" into their own "cancelled" bucket, outside the show rate. That was accurate and still wrong: Ron's definition is that a booked call which does not happen is a no-show, whoever cancelled, so the honest-looking neutral bucket was flattering the show rate by 14 points. The data can tell you a row is not pending; it cannot tell you what it counts as. Two guards made the rule safe to apply literally, and both came from reading the rows rather than the rule: "cancelled" in the snapshot is two different events (a webhook cancellation vs. a deletion sweep over QA bookings and duplicates), and a cancellation followed by a new booking is a reschedule, which would otherwise score one lead as a no-show plus a call. Same session, second lesson: a number only means what its collection path can see. "Unread in GHL" was computed correctly in #210 and removed a day later, because setters answer from the WhatsApp phone app and GHL never learns the message was read. Before publishing a per-person metric, ask where the people actually do the work. And a process note: a prompt stored in the database but loaded at boot is deployed by the NEXT deploy, not by the UPDATE, so write it immediately before the merge that needs it.
- **2026-09-22: a large, steady skip bucket is a rule gap, not noise ([PR #217](https://github.com/neurogrowth-cr/ng-agent/pull/217)).** Twice in August a "the mover is broken" complaint was a visibility problem and the strike_sweep rows proved the code right, so the reflex became "check the audit rows and answer with counts". Third complaint, same reflex, different answer: the rows showed `no_whatsapp: 58` on every sweep for weeks. That bucket was named after the assumption baked into the filter (WhatsApp is the only channel), and every Messenger lead, every GHL email, and every logged call fell into it. An audit trail confirms what the code did; it cannot tell you the code is asking the right question. When a skip reason is a constant fraction of every run, open a few of those cards and read them as a setter would. Second lesson from the same fix: a pure decision function is only as good as the data reaching it. Emails sent through Max never touched GHL at all, so no rule change could count them; the fix had to widen the input (a table read merged into the message history), not the rule.
