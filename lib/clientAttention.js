  ...
  if (positives.length > 0 || urgentOthers.length > 0) {
    ...
    if (positives.length > 0) {
      ...
      const top = positives
        ...
        .join(', ');
      lines.push(
        `🔴 Positive replies waiting over 24h: ${plural(positives.length, 'client', 'clients')}, ${plural(total, 'reply', 'replies')}. Most: ${top} → Answer them in LinkedIn today. Answers are picked up within the hour.`
      );
      budget -= 1;
    }
